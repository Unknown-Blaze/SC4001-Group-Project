...
from metrics_utils import compute_classification_metrics, predict_probs
...
def build_amazon_30k(amazon, total=30000, adapt_ratio=2.0/3.0, seed=42):
    ...
    adapt_labels = labels_sel[:n_adapt]  # labels unused for TTA, but kept for debugging
    eval_texts = texts_sel[n_adapt:]
    eval_labels = labels_sel[n_adapt:]

    print(f"[AMAZON] 30k subset: {len(adapt_texts)} adapt, {len(eval_texts)} eval "
...
def train_one_epoch(model, dataloader, optimizer, scheduler, cfg: TrainConfig, desc: str):
    model.train()
    total_loss = 0.0
...
@torch.no_grad()
def evaluate_model(model, dataloader, desc: str):
    model.eval()
    ...
    return compute_classification_metrics(all_labels, all_probs, num_labels=2)


def evaluate_on_texts(model, texts, labels, tokenizer,
                      max_length=256, batch_size=64, tag=""):
    ds = TextDataset(texts, labels, tokenizer, max_length=max_length)
    loader = DataLoader(ds, batch_size=batch_size, shuffle=False)
    print(f"[Eval] Evaluating on {tag} ({len(texts)} samples)...", flush=True)
...
# =========================
# Optuna + K-fold for domain adapters
# =========================
...
def tune_and_train_domain_adapter(
    ...
    """
    Tune LoRA + training hyperparams for one domain on its 30k TRAIN subset,
    then train final adapter on that train subset.
    """
    set_seed(42)
    ...
    def objective(trial):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(skf.split(texts, labels)):
            print(f"[{domain_name.upper()}][Trial {trial.number}] Fold {fold_idx+1}/{n_splits}...", flush=True)
            ...
            for epoch in range(cfg.num_epochs):
                desc = f"[{domain_name}][T{trial.number}][F{fold_idx+1}] Ep {epoch+1}/{cfg.num_epochs}"
                loss = train_one_epoch(model, train_loader, optimizer, scheduler, cfg, desc)
                print(f"[{domain_name.upper()}][Trial {trial.number}] "
                      f"Fold {fold_idx+1} Ep {epoch+1} Loss={loss:.4f}", flush=True)

            metrics = evaluate_model(
                model,
                val_loader,
                desc=f"[{domain_name}][T{trial.number}][F{fold_idx+1}] Val",
            )
            print(f"[{domain_name.upper()}][Trial {trial.number}] "
                  f"Fold {fold_idx+1} Val macro-F1={metrics['macro_f1']:.4f}", flush=True)
        ...
        print(f"[{domain_name.upper()}][Trial {trial.number}] Mean CV macro-F1={mean_f1:.4f}", flush=True)
        return mean_f1
    ...
    # Train final adapter on entire (30k *train subset*) passed in
    full_ds = TextDataset(texts.tolist(), labels.tolist(), tokenizer,
    ...
    for epoch in range(best_cfg.num_epochs):
        ...
        loss = train_one_epoch(model, full_loader, optimizer, scheduler, best_cfg, desc)
        print(f"[{domain_name.upper()}][FINAL] Ep {epoch+1} Loss={loss:.4f}", flush=True)
...
# =========================
# Fusion tuning and training on 30k subsets
# =========================
...
def tune_and_train_fusion(
    ...
    def objective(trial):
        ...
        print(f"[Fusion][Trial {trial.number}] "
              f"hd={hidden_dim}, drop={gate_dropout:.3f}, "
              f"lr={lr:.2e}, wd={weight_decay:.2e}, "
              f"ep={num_epochs}, bs={batch_size}", flush=True)
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader, desc=desc, leave=False):
                ...
                out = fusion_model(input_ids=input_ids,
                                   attention_mask=attention_mask,
                                   labels=labels)
                loss = out["loss"]
                loss.backward()
            ...
            print(f"[Fusion][Trial {trial.number}] Ep {epoch+1} Loss={epoch_loss:.4f}", flush=True)
        ...
        print(f"[Fusion][Trial {trial.number}] Val macro-F1={metrics['macro_f1']:.4f}", flush=True)
    ...
    # Train final fusion on full pooled train
    ...
    for epoch in range(num_epochs):
        ...
        print(f"[Fusion][FINAL] Ep {epoch+1} Loss={total_loss:.4f}", flush=True)
...
# =========================
# TENT-style gate adaptation on Amazon 30k subset
# =========================

def tent_entropy_minimization(
    fusion_model,
    adapt_texts,
    batch_size: int = 32,
    lr: float = 1e-4,
    ...
    print("[TTA] Starting TENT-style adaptation on Amazon adapt subset...", flush=True)
    tokenizer = get_e5_tokenizer()
    ...
    ds = AmazonAdaptDataset(adapt_texts, tokenizer)
    ...
    for step in range(steps):
        step_loss = 0.0
        for batch in tqdm(loader, desc=f"[TTA] Step {step+1}/{steps}", leave=False):
            optimizer.zero_grad()
            input_ids = batch["input_ids"].to(DEVICE)
            attention_mask = batch["attention_mask"].to(DEVICE)
        ...
        step_loss /= len(ds)
        print(f"[TTA] Step {step+1} entropy loss={step_loss:.6f}", flush=True)

    print("[TTA] Finished TENT adaptation.", flush=True)
...
def main():
    ...
    # 1) Train per-domain adapters on 30k-train subsets
    imdb_dir, imdb_cfg = tune_and_train_domain_adapter(
        "imdb",
        imdb_train_texts,
        imdb_train_labels,
        n_splits=3,
    ...
    yelp_dir, yelp_cfg = tune_and_train_domain_adapter(
        "yelp",
        yelp_train_texts,
        yelp_train_labels,
        n_splits=3,
    ...
    # 2) Fusion on pooled IMDB+Yelp train subsets
    ...
    # 3) Evaluate fusion (pre-TTA) on IMDB/Yelp eval subsets (all within 30k budgets)
    imdb_metrics = evaluate_on_texts(
        fusion_model,
        imdb_eval_texts,
        imdb_eval_labels,
        tokenizer,
    ...
    yelp_metrics = evaluate_on_texts(
        fusion_model,
        yelp_eval_texts,
        yelp_eval_labels,
        tokenizer,
    ...
    amazon_before = evaluate_on_texts(
        fusion_model,
        amazon_eval_texts,
        amazon_eval_labels,
        tokenizer,
    ...
    metrics["fusion_imdb_eval"] = imdb_metrics
    metrics["fusion_yelp_eval"] = yelp_metrics
    metrics["fusion_amazon_before_tta"] = amazon_before

    # 4) TENT-style adaptation on Amazon adapt subset (20k)
    tent_entropy_minimization(
        fusion_model,
        amazon_adapt_texts,
        batch_size=32,
        lr=7.5e-5,
    ...
    # 5) Evaluate fusion (post-TTA) on Amazon eval subset (10k)
    amazon_after = evaluate_on_texts(
        fusion_model,
        amazon_eval_texts,
        amazon_eval_labels,
        tokenizer,
        tag="Amazon eval post-TTA (subset of 30k)",
    )
    metrics["fusion_amazon_after_tta"] = amazon_after

    # Save metrics & adapted fusion
    with open(os.path.join("outputs", "method_b_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=2)

    torch.save(
        fusion_model.state_dict(),
        os.path.join(fusion_dir, "fusion_model_tta.pt"),
    )
